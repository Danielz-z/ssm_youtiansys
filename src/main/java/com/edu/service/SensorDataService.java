 ...
 * 本文件对应Service部分，负责表SensorData
 * 箭头结构：JSP->Controller->[Service]->Mapper->数据库-->JSP
 */
public interface SensorDataService {//service接口，使用的原因倒不是必要，本来Controller也可以自动调用数据库的Mapper进行操作数据库，但是如果项目代码比较多，直接调用mapper会导致controller中的代码比较多，这里写service的原因就是为了可以为controller层减轻压力，为后续扩充做准备。接口的学习文档https://www.runoob.com/java/java-interfaces.html
	...
	public List<SensorData> getList(String field,
