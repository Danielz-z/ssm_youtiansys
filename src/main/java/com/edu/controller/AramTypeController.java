...
/**
 * author:dyb1296
 * time:2024-4-15 0:34:32
 * email:dyb1296@qq.com
 * function:负责报警类型功能，这个是MVC的C层，也就是控制层，这个Controller类可以实现对AramType这个类的控制，接收浏览器发送的请求并处理。AramType类的具体介绍，在com.edu.model.AramType类下，本处不在赘述
...
@RequestMapping(value = "AramType")//定义请求的路径，路径为AramType，定义之后，浏览器获取数据就根据这个路径来进行访问，其他的同理，只要有@RequestMapping注解的都是这样
public class AramTypeController {//Controller包含在名字中是一种规范
	@Autowired//自动注入，Spring的注解，这样的话，程序就可以自动把这个类扫进来，不需要手动创建对象
	private AramTypeService aramTypeService;//调用数据库mapper，mybatis框架的内容，可以实现增删改查

	@RequestMapping(value = "/initPage.do")//设置添加跳转的页面
	public String initPage(HttpServletRequest request, Model model) {//在点击添加的时候加载一些预置代码
		return "AramType/saveOrUpdate";//跳转到编辑报警类型页面
	}

	@RequestMapping(value = "/selectList.do")//新增或者编辑报警类型的时候调用，然后跳转到编辑页&添加报警类型页面
	public String selectList(HttpServletRequest request, AramType aramType, Model model) {//获取列表，最后跳转到报警类型列表查询页面
		aramType = aramTypeService.getById(aramType.getId());//获取要更新的对象
		model.addAttribute("util", aramType);//传到前台
		return "AramType/saveOrUpdate";//跳转到编辑页面
	}
	...
	public String getAllDataInPage(HttpServletRequest request, Model model) {
		...
		List<AramType> list = aramTypeService.getList(field, fieldValue);//获取列表数据
		...
		return "AramType/find";//跳转到find.jsp页面，也就是列表页面，AramType文件夹下面	
}

	@RequestMapping(value = "/deleteUtil.do")
	public String deleteUtil(HttpServletRequest request, AramType aramType, Model model) {//删除的代码
		try{
			aramType = aramTypeService.getById(aramType.getId());
			if(aramType == null){//如果这个数据已经不存在了，那么就跳转到列表页面
				return this.getAllDataInPage(request, model);
			}
			aramTypeService.deleteById(aramType.getId());//根据ID删除
		}catch(Exception e){
			model.addAttribute("msg", "<script>alertHui(\"删除失败\",\"有其他的数据依赖该数据，删除失败，请删除该数据-->'"+aramType.getStype()+"'关联的数据后在删除本数据！\");</script>");
			return this.getAllDataInPage(request, model);//返回到列表页面，对应admin文件夹下的AramType文件夹下的find.jsp
		}
		return this.getAllDataInPage(request, model);//返回到列表页面，对应admin文件夹下的AramType文件夹下的find.jsp
	}
	...
	public String deleteManyDataByIds(HttpServletRequest request, Model model) {                                                 
		...
		for (String id : ids) {//考虑可能传多个id                                          
			try{
				aramTypeService.deleteById(Integer.parseInt(id));//删除的核心代码
			}catch(Exception e){}
		}                                                                
		return this.getAllDataInPage(request, model);                    
	}                                                                  
	...
	public String saveOrupdate(HttpServletRequest request, AramType util, Model model) {
		List<AramType> list = aramTypeService.getList("stype", util.getStype());
		if (0 == util.getId()) {

			aramTypeService.insert(util);
		} else {

			aramTypeService.update(util);
		}
		return this.getAllDataInPage(request, model);
	}
	...
	public void export(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {//获取列表，最后跳转到列表查询页面，获取的是
		...
		List<AramType> list = aramTypeService.getList(field, fieldValue);//获取列表数据
		Class p= AramType.class;
		Field[] fields = p.getDeclaredFields();
		for(AramType aramType:list) {
...
//				Tip tip = declaredField.getAnnotation(Tip.class);
					...
					if(name == null){
 					...
 					if(declaredField.getType().toString().equals("class java.lang.String")){
						row2.put(name, declaredField.get(aramType));
...
//		fileName = URLEncoder.encode(fileName,"UTF-8");
	...
	@RequestMapping(value = "/showAramType.do")//这个是跳转到展示数据的页面，有些页面需要展示数据
	public String showAramType(HttpServletRequest request, Integer id, Model model) {
		HttpSession session = request.getSession();
		if(session.getAttribute("user_id") == null){
			model.addAttribute("errMsg", "超时请重新登录");
			return "../login";
		}
		AramType aramType = aramTypeService.getById(id);
		model.addAttribute("aramType", aramType);
		return "AramType/show";
	}
	@SuppressWarnings({ "rawtypes", "unchecked" })
	...
	public String getAllView(HttpServletRequest request, Model model) {
		...
		List<AramType> list = aramTypeService.getList("", "");//获取列表数据
		PageModel pageModel = new PageModel();
		pageModel = pageModel.getUtilByController(list, currentPageNo);
		model.addAttribute("pageModel", pageModel);//查询后，把查询的条件类型回显到jsp中

		return "AramType/viewlist";//跳转到前台的列表
	}
	@RequestMapping(value = "/view.do")
	public String view(HttpServletRequest request, AramType aramType, Model model) {//把页面的数据显示上来
		aramType = aramTypeService.getById(aramType.getId());//获取要更新的对象
		model.addAttribute("obj", aramType);//传到前台
		return "AramType/view";//跳转到编辑页面
	}
}
