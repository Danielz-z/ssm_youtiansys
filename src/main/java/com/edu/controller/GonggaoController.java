...
/**
 * author:dyb1296
 * time:2024-4-10 14:15:16
 * email:dyb1296@qq.com
 * function:负责公告管理功能，这个是MVC的C层，也就是控制层，这个Controller类可以实现对Gonggao这个类的控制，接收浏览器发送的请求并处理。Gonggao类的具体介绍，在com.edu.model.Gonggao类下，本处不在赘述
...
@RequestMapping(value = "Gonggao")//定义请求的路径，路径为Gonggao，定义之后，浏览器获取数据就根据这个路径来进行访问，其他的同理，只要有@RequestMapping注解的都是这样
public class GonggaoController {//Controller包含在名字中是一种规范
	@Autowired//自动注入，Spring的注解，这样的话，程序就可以自动把这个类扫进来，不需要手动创建对象
	private GonggaoService gonggaoService;//调用数据库mapper，mybatis框架的内容，可以实现增删改查

	@RequestMapping(value = "/initPage.do")//设置添加跳转的页面
	public String initPage(HttpServletRequest request, Model model) {//在点击添加的时候加载一些预置代码
		return "Gonggao/saveOrUpdate";//跳转到编辑公告页面
	}

	@RequestMapping(value = "/selectList.do")//新增或者编辑公告的时候调用，然后跳转到编辑页&添加公告页面
	public String selectList(HttpServletRequest request, Gonggao gonggao, Model model) {//获取列表，最后跳转到公告列表查询页面
		gonggao = gonggaoService.getById(gonggao.getId());//获取要更新的对象
		model.addAttribute("util", gonggao);//传到前台
		return "Gonggao/saveOrUpdate";//跳转到编辑页面
	}
	...
	public String getAllDataInPage(HttpServletRequest request, Model model) {
		...
		List<Gonggao> list = gonggaoService.getList(field, fieldValue);//获取列表数据
		...
		return "Gonggao/find";//跳转到find.jsp页面，也就是列表页面，Gonggao文件夹下面	
}

	@RequestMapping(value = "/deleteUtil.do")
	public String deleteUtil(HttpServletRequest request, Gonggao gonggao, Model model) {//删除的代码
		try{
			gonggao = gonggaoService.getById(gonggao.getId());
			if(gonggao == null){//如果这个数据已经不存在了，那么就跳转到列表页面
				return this.getAllDataInPage(request, model);
			}
			gonggaoService.deleteById(gonggao.getId());//根据ID删除
		}catch(Exception e){
			model.addAttribute("msg", "<script>alertHui(\"删除失败\",\"有其他的数据依赖该数据，删除失败，请删除该数据-->'"+gonggao.getTitle()+"'关联的数据后在删除本数据！\");</script>");
			return this.getAllDataInPage(request, model);//返回到列表页面，对应admin文件夹下的Gonggao文件夹下的find.jsp
		}
		return this.getAllDataInPage(request, model);//返回到列表页面，对应admin文件夹下的Gonggao文件夹下的find.jsp
	}
	...
	public String deleteManyDataByIds(HttpServletRequest request, Model model) {                                                 
		...
		for (String id : ids) {//考虑可能传多个id                                          
			try{
				gonggaoService.deleteById(Integer.parseInt(id));//删除的核心代码
			}catch(Exception e){}
		}                                                                
		return this.getAllDataInPage(request, model);                    
	}                                                                  
	...
	public String saveOrupdate(HttpServletRequest request, Gonggao util, Model model) {
		List<Gonggao> list = gonggaoService.getList("title", util.getTitle());
		if (0 == util.getId()) {

			gonggaoService.insert(util);
		} else {

			gonggaoService.update(util);
		}
		return this.getAllDataInPage(request, model);
	}
...
//				Tip tip = declaredField.getAnnotation(Tip.class);
					...
					if(name == null){
 					...
 					if(declaredField.getType().toString().equals("class java.lang.String")){
						row2.put(name, declaredField.get(gonggao));
		...
		try {
			...
			writer.addHeaderAlias("标题", "标题");
			writer.addHeaderAlias("内容", "内容");
			writer.addHeaderAlias("发布时间", "发布时间");
...
//		fileName = URLEncoder.encode(fileName,"UTF-8");
	...
	@RequestMapping(value = "/showGonggao.do")//这个是跳转到展示数据的页面，有些页面需要展示数据
	public String showGonggao(HttpServletRequest request, Integer id, Model model) {
	...
	public String getAllView(HttpServletRequest request, Model model) {
		...
		List<Gonggao> list = gonggaoService.getList("", "");//获取列表数据
		PageModel pageModel = new PageModel();
		pageModel = pageModel.getUtilByController(list, currentPageNo);
		model.addAttribute("pageModel", pageModel);//查询后，把查询的条件类型回显到jsp中

		return "Gonggao/viewlist";//跳转到前台的列表
	}
	@RequestMapping(value = "/view.do")
	public String view(HttpServletRequest request, Gonggao gonggao, Model model) {//把页面的数据显示上来
		gonggao = gonggaoService.getById(gonggao.getId());//获取要更新的对象
		model.addAttribute("obj", gonggao);//传到前台
		return "Gonggao/view";//跳转到编辑页面
	}
}
