 ...
 * 本文件对应Mapper，负责表SafetyInfo
 * 箭头结构：JSP->Controller->Service->[Mapper]->数据库-->JSP
 */
public interface SafetyInfoMapper {//Mapper结尾是为了规范，当看见XXXMapper时候就可以意识到这个是操作数据库用的，执行对应的代码直接实现增删改查，不需要手动拼接数据库语句，非常的方便
	...
	public List<SafetyInfo> getList(@Param("field") String field,
